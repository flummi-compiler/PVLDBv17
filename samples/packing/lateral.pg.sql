...
(
    (SELECT True,
      ...
      FROM LATERAL
          (WITH "let36"("n_1") AS MATERIALIZED(
            (SELECT (SELECT (count(*)) :: int4 AS "count"
                      FROM lineitem AS "l"
                      WHERE "l"."l_orderkey" = o."o_orderkey") AS "n_1")
          ) SELECT * FROM "let36") AS "let36"("n_1"),
          LATERAL (SELECT "n_1" = 0 AS "q4_1") AS "let37"("q4_1"),
          LATERAL
          (WITH "let39"("q8_2") AS MATERIALIZED (
            SELECT "capacity"
                      <
          ...
          ) SELECT * FROM "let39") AS "let39"("q8_2")
    ...
    (SELECT "result".*
      FROM run AS "run"("rec?",
          ...
          ((SELECT "ifresult2".*
            ...
            ((SELECT "ifresult23".*
              FROM LATERAL
                  (WITH "let21"("size_6") AS MATERIALIZED (
                      (SELECT (SELECT (sum("p"."p_size")) :: int4 AS "sum"
                                ...
                                WHERE ("l"."l_orderkey" = "orderkey"
                                    ...
                                    AND
                                    "l"."l_partkey" = "p"."p_partkey")) AS "size_6")
                  ) SELECT * FROM "let21") AS "let21"("size_6"),
                  LATERAL
