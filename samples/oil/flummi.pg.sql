...
WITH RECURSIVE
  "%loop%"("%kind%", "%label%", "cost", "pivot_x", "pivot_y", "slope", "well", "yield", "%result%") AS (
    (SELECT 'jump' AS "%kind%",
            ...
            CAST(NULL AS well) AS "well",
            CAST(NULL AS int) AS "yield",
            CAST(NULL AS well) AS "%result%")
    ...
    (WITH
      ...
      "entry"("%kind%", "%label%", "cost", "pivot_x", "pivot_y", "slope", "well", "yield", "%result%") AS (
        WITH
          ...
          "%assign%"("cost", "pivot_x", "pivot_y", "slope", "well", "yield") AS (
            SELECT CAST(('-Infinity' :: float) AS float) AS "cost",
                   ...
                   CAST((("%inputs%"."well")) AS well) AS "well",
                   CAST((SELECT abs(p.c)
                         FROM   endpoints AS p
        ...
        SELECT 'goto', 'loop_head', "cost", "pivot_x", "pivot_y", "slope", "well", "yield", CAST(NULL AS well)
        FROM   "%assign%"
        WHERE  TRUE
      ),
      "loop_head"("%kind%", "%label%", "current", "pivot_x", "pivot_y", "well", "yield", "%result%") AS (
        WITH
          ...
          "%assign%"("current", "pivot_x", "pivot_y", "well", "yield") AS (
            SELECT CAST((SELECT  (e.x, e.y, actual_cost, rot) :: bore
                         FROM    endpoints AS e,
        ...
        SELECT 'goto', 'inter3', "current", "pivot_x", "pivot_y", "well", "yield", CAST(NULL AS well)
        FROM   "%assign%"
        WHERE  TRUE
      ),
      "inter3"("%kind%", "%label%", "current", "pivot_x", "pivot_y", "well", "yield", "%result%") AS (
        WITH
          ...
          "%assign%"("current", "pivot_x", "pivot_y", "well", "yield", "condition%0") AS (
            SELECT CAST((("%inputs%"."current")) AS bore) AS "current",
                   CAST((("%inputs%"."pivot_x")) AS int) AS "pivot_x",
                   CAST((("%inputs%"."pivot_y")) AS int) AS "pivot_y",
                   CAST((("%inputs%"."well")) AS well) AS "well",
                   CAST((("%inputs%"."yield")) AS int) AS "yield",
        ...
        SELECT 'goto', 'falsey0', "current", "pivot_x", "pivot_y", "well", "yield", CAST(NULL AS well)
        FROM   "%assign%"
        WHERE  TRUE AND NOT "condition%0"
          UNION ALL
        SELECT 'goto', 'truthy0', "current", "pivot_x", "pivot_y", "well", "yield", CAST(NULL AS well)
        FROM   "%assign%"
      ...
      "falsey0"("%kind%", "%label%", "cost", "current", "pivot_x", "pivot_y", "slope", "well", "yield", "%result%") AS (
        WITH
          ...
          "%assign%"("cost", "current", "pivot_x", "pivot_y", "slope", "well", "yield") AS (
            SELECT CAST(((("%inputs%"."current")).cost) AS float) AS "cost",
                   CAST((("%inputs%"."current")) AS bore) AS "current",
                   CAST((("%inputs%"."pivot_x")) AS int) AS "pivot_x",
                   CAST((("%inputs%"."pivot_y")) AS int) AS "pivot_y",
        ...
        SELECT 'goto', 'merge0', "cost", "current", "pivot_x", "pivot_y", "slope", "well", "yield", CAST(NULL AS well)
        FROM   "%assign%"
        WHERE  TRUE
      ),
      "merge0"("%kind%", "%label%", "cost", "current", "pivot_x", "pivot_y", "slope", "well", "yield", "%result%") AS (
        ...
        SELECT 'goto', 'inter6', "cost", "current", "pivot_x", "pivot_y", "slope", "well", "yield", CAST(NULL AS well)
        FROM   "%assign%"
        WHERE  TRUE
      ),
      "inter6"("%kind%", "%label%", "cost", "current", "pivot_x", "pivot_y", "slope", "well", "yield", "%result%") AS (
        ...
        SELECT 'goto', 'truthy1', "cost", "current", "pivot_x", "pivot_y", "slope", "well", "yield", CAST(NULL AS well)
        FROM   "%assign%"
        WHERE  TRUE AND "condition%1"
          UNION ALL
        SELECT 'jump', 'loop_head', "cost", "current", "pivot_x", "pivot_y", "slope", "well", "yield", CAST(NULL AS well)
        FROM   "%assign%"
        ...
        WITH
          ...
          "%assign%"("cost", "pivot_x", "pivot_y", "slope", "well", "yield") AS (
            SELECT CAST((("%inputs%"."cost")) AS float) AS "cost",
                   ...
                   CAST((("%inputs%"."pivot_y")) AS int) AS "pivot_y",
                   CAST((("%inputs%"."slope")) AS float) AS "slope",
                   CAST((((("%inputs%"."current")).x, (("%inputs%"."current")).y, ("%inputs%"."yield")) :: well) AS well) AS "well",
                   CAST((("%inputs%"."yield")) AS int) AS "yield"
            FROM "%inputs%"
          )

        SELECT 'jump', 'loop_head', "cost", "pivot_x", "pivot_y", "slope", "well", "yield", CAST(NULL AS well)
        FROM   "%assign%"
        WHERE  TRUE
      )

     SELECT 'jump', "%label%", "cost", "pivot_x", "pivot_y", "slope", "well", "yield", CAST(NULL AS well)
     FROM   "inter6"
     WHERE  "%kind%"='jump'
       UNION ALL
     SELECT 'jump', "%label%", "cost", "pivot_x", "pivot_y", "slope", "well", "yield", CAST(NULL AS well)
     ...
     WHERE  "%kind%"='jump'
       UNION ALL
     SELECT 'emit', NULL, CAST(NULL AS float), CAST(NULL AS int), CAST(NULL AS int), CAST(NULL AS float), CAST(NULL AS well), CAST(NULL AS int), "%result%"
     FROM   "truthy0"
     WHERE  "%kind%"='emit'
    )
  )
