...
WITH RECURSIVE
  "%loop%"("%kind%", "%label%", "blame", "current_item", "multi", "orderkey", "suppkey", "%result%") AS (
    ...
    (WITH
      ...
      "falsey0"("%kind%", "%label%", "blame", "current_item", "lineitem", "multi", "orderkey", "suppkey", "%result%") AS (
        WITH
          ...
          "%assign%"("blame", "current_item", "lineitem", "multi", "orderkey", "suppkey") AS (
            SELECT CAST((("%inputs%"."blame")) AS bool) AS "blame",
                   CAST((("%inputs%"."current_item")) AS int) AS "current_item",
                   CAST((SELECT l.l_suppkey << 1 | (l.l_receiptdate > l.l_commitdate) :: int
                         FROM   lineitem AS l
                         WHERE  l.l_orderkey = ("%inputs%"."orderkey")
                         AND    l.l_linenumber = ("%inputs%"."current_item")) AS int) AS "lineitem",
                   CAST((("%inputs%"."multi")) AS bool) AS "multi",
                   CAST((("%inputs%"."orderkey")) AS int) AS "orderkey",
                   CAST((("%inputs%"."suppkey")) AS int) AS "suppkey"
            FROM "%inputs%"
      ...
      "inter4"("%kind%", "%label%", "blame", "current_item", "l_suppkey", "multi", "orderkey", "receipt_gt_commit", "suppkey", "%result%") AS (
        WITH
          ...
          "%assign%"("blame", "current_item", "l_suppkey", "multi", "orderkey", "receipt_gt_commit", "suppkey") AS (
            SELECT CAST((("%inputs%"."blame")) AS bool) AS "blame",
                   CAST((("%inputs%"."current_item")) AS int) AS "current_item",
                   CAST((("%inputs%"."lineitem") >> 1) AS int) AS "l_suppkey",
                   CAST((("%inputs%"."multi")) AS bool) AS "multi",
                   CAST((("%inputs%"."orderkey")) AS int) AS "orderkey",
                   CAST((("%inputs%"."lineitem") & 1 = 1) AS bool) AS "receipt_gt_commit",
                   CAST((("%inputs%"."suppkey")) AS int) AS "suppkey"
            FROM "%inputs%"
          )

        SELECT 'goto', 'inter5', "blame", "current_item", "l_suppkey", "multi", "orderkey", "receipt_gt_commit", "suppkey", CAST(NULL AS bool)
        FROM   "%assign%"
        WHERE  TRUE
      ),
      "inter5"("%kind%", "%label%", "blame", "current_item", "multi", "orderkey", "suppkey", "%result%") AS (
        WITH
          "%inputs%"("blame", "current_item", "l_suppkey", "multi", "orderkey", "receipt_gt_commit", "suppkey") AS (
            SELECT "blame", "current_item", "l_suppkey", "multi", "orderkey", "receipt_gt_commit", "suppkey"
            FROM   "inter4"
            WHERE  "%kind%"='goto'
            AND    "%label%"='inter5'
          ),
          "%assign%"("blame", "current_item", "multi", "orderkey", "suppkey", "condition%1", "condition%2") AS (
            SELECT CAST((("%inputs%"."blame")) AS bool) AS "blame",
                   CAST((("%inputs%"."current_item")) AS int) AS "current_item",
                   CAST((("%inputs%"."multi") OR (("%inputs%"."l_suppkey") <> ("%inputs%"."suppkey"))) AS bool) AS "multi",
                   CAST((("%inputs%"."orderkey")) AS int) AS "orderkey",
                   CAST((("%inputs%"."suppkey")) AS int) AS "suppkey",
                   CAST((("%inputs%"."receipt_gt_commit")) AS bool) AS "condition%1",
                   CAST((("%inputs%"."l_suppkey") <> ("%inputs%"."suppkey")) AS bool) AS "condition%2"
            FROM "%inputs%"
          )
