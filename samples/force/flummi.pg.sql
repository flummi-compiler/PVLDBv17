...
WITH RECURSIVE
  "%loop%"("%kind%", "%label%", "G", "Q", "body", "force", "theta", "%result%") AS (
    (SELECT 'jump' AS "%kind%",
            ...
            CAST(ROW(starting_point.xy, 1.0) AS bodies) AS "body",
    ...
    (WITH
        ...
        WITH
          ...
          "%assign%"("G", "body", "force", "node", "theta") AS (
            SELECT CAST((6.67e-11) AS float      ) AS "G",
                   CAST((("%inputs%"."body")) AS bodies) AS "body",
        ...
        WITH
          ...
          "%assign%"("G", "Q", "body", "dir", "dist", "force", "node", "theta", "condition%0") AS (
            SELECT CAST((("%inputs%"."G")) AS float      ) AS "G",
                   ...
                   CAST((("%inputs%"."body")) AS bodies) AS "body",
                   CAST(((("%inputs%"."node")).center - (("%inputs%"."body")).pos) AS point      ) AS "dir",
                   CAST((GREATEST((("%inputs%"."node")).center <-> (("%inputs%"."body")).pos, 1e-10)) AS float      ) AS "dist",
                   CAST((("%inputs%"."force")) AS point      ) AS "force",
                   CAST((("%inputs%"."node")) AS barneshut  ) AS "node",
                   CAST((("%inputs%"."theta")) AS float) AS "theta",
                   CAST((NOT EXISTS (SELECT 1
                                     FROM   walls AS w
                                     WHERE  ((("%inputs%"."body")).pos    <= (("%inputs%"."body")).pos    ## w.wall) <>
                                            ((("%inputs%"."node")).center <= (("%inputs%"."node")).center ## w.wall))) AS bool) AS "condition%0"
            FROM "%inputs%"
          )
        ...
        WITH
          ...
          "%assign%"("G", "Q", "body", "force", "grav", "node", "theta", "condition%1") AS (
            SELECT CAST((("%inputs%"."G")) AS float      ) AS "G",
                   ...
                   CAST((("%inputs%"."body")) AS bodies) AS "body",
                   CAST((("%inputs%"."force")) AS point      ) AS "force",
                   CAST(((("%inputs%"."G") * (("%inputs%"."body")).mass * (("%inputs%"."node")).mass / ("%inputs%"."dist")^2) * ("%inputs%"."dir")) AS point      ) AS "grav",
                   CAST((("%inputs%"."node")) AS barneshut  ) AS "node",
                   CAST((("%inputs%"."theta")) AS float) AS "theta",
        ...
        WITH
          ...
          "%assign%"("G", "Q", "body", "force", "theta", "condition%3") AS (
            SELECT CAST((("%inputs%"."G")) AS float      ) AS "G",
                   CAST((("%inputs%"."Q")) AS barneshut[]) AS "Q",
                   CAST((("%inputs%"."body")) AS bodies) AS "body",
                   CAST((("%inputs%"."force") + ("%inputs%"."grav")) AS point      ) AS "force",
                   CAST((("%inputs%"."theta")) AS float) AS "theta",
                   CAST((CARDINALITY(("%inputs%"."Q")) > 0) AS bool) AS "condition%3"
            FROM "%inputs%"
          )
      ...
      "falsey1"("%kind%", "%label%", "G", "Q", "body", "force", "theta", "%result%") AS (
        WITH
          ...
          "%assign%"("G", "Q", "body", "force", "theta") AS (
            SELECT CAST((("%inputs%"."G")) AS float      ) AS "G",
                   CAST((SELECT ("%inputs%"."Q") || ARRAY_AGG(b)
                         FROM   barneshut AS b
        ...
        WITH
          ...
          "%assign%"("G", "Q", "body", "force", "theta", "condition%4") AS (
            SELECT CAST((("%inputs%"."G")) AS float      ) AS "G",
                   ...
                   CAST((("%inputs%"."force")) AS point      ) AS "force",
                   CAST((("%inputs%"."theta")) AS float) AS "theta",
                   CAST((CARDINALITY(("%inputs%"."Q")) > 0) AS bool) AS "condition%4"
            FROM "%inputs%"
          )
     ...
     WHERE  "%kind%"='jump'
       UNION ALL
     SELECT 'emit', NULL, CAST(NULL AS float      ), CAST(NULL AS barneshut[]), CAST(NULL AS bodies), CAST(NULL AS point      ), CAST(NULL AS float), "%result%"
     FROM   "falsey2"
     WHERE  "%kind%"='emit'
    )
  )
